...
(port 8080) and proxies third-party APIs (Last.fm) and serves travel pin data.
...
- `CLOUDINARY_CLOUD_NAME` — required for travel pins
- `CLOUDINARY_API_KEY` — required for travel pins
- `CLOUDINARY_API_SECRET` — required for travel pins
...
Example: `lastfmService.ts` exports `getRecentTracks()`.
...
e.g. `AboutPage/index.tsx`, `MusicPage/index.tsx`.
...
Services receive infrastructure clients (e.g. `*cloudinary.CloudinaryService`) via constructor injection.
...
- Each component that needs styles gets an `index.css` inside its folder (e.g. `Terminal/index.css`).
- CSS class names use `kebab-case` prefixed with a short component abbreviation (e.g. `.tmap-header`, `.scrobble-nav`).
...
|---|---|---|
| Service files | `camelCase.ts` | `lastfmService.ts` |
| Hook files | `camelCase.ts` | `useLastFM.ts` |
| Data files | `camelCase.ts` | `blogPosts.ts` |
| Component folders | `PascalCase/` | `ScrobbleCarousel/` |
| Component files | `index.tsx` | `ScrobbleCarousel/index.tsx` |
| Component CSS | `index.css` | `ScrobbleCarousel/index.css` |
| React components | `PascalCase` | `ScrobbleCarousel` |
| Hooks | `useCamelCase` | `useLastFM` |
| CSS classes | `kebab-case`, component-prefixed | `.scrobble-nav` |
...
- **The Go backend is a proxy/API**, not a database. Keep handlers stateless.
